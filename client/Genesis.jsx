...
/* ═══════════════════════════════════════════════════════════════
   GENESIS — Public AI World Viewer
...
// ─── ZONE COLORS ───
const ZC = {
  nexus: "#00e5ff", deep_archive: "#9c7cff", signal_edge: "#ff6e40",
  null_zone: "#546e7a", core: "#69f0ae", flux_field: "#ffd740",
};
const LC = {
  system: "#00e5ff", world: "#ffd740", action: "#5a5a70", creation: "#69f0ae",
  evolve: "#ce93d8", trait: "#ffab40", build: "#80cbc4", message: "#81d4fa",
  death: "#ef5350", move: "#546e7a", thought: "#6a6a80",
};
...
function Sparkline({ data, width = 140, height = 28, color = "#69f0ae" }) {
  ...
  return (
    <svg width={width} height={height} style={{ display: "block" }}>
      ...
      <circle cx={width} cy={height - (data[data.length - 1] / max) * (height - 4)} r="2.5" fill={color} />
    </svg>
  );
}

// ─── ZONE CARD ───
function ZoneCard({ zk, zone, agents, onSelect }) {
  const here = agents.filter(a => a.alive && a.zone === zk);
  const color = ZC[zk] || "#888";
  return (
    <div style={{
      background: `${color}06`, borderRadius: 10, padding: "10px 12px",
      border: `1px solid ${color}15`, minHeight: 64,
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
        <span style={{ fontSize: 9, color, letterSpacing: 2, fontWeight: 700, opacity: 0.7 }}>{zone.label}</span>
        <span style={{ fontSize: 8, color: "#5a5a6a" }}>{here.length}</span>
      </div>
      <div style={{ fontSize: 8, color: "#5a5a6a", marginBottom: 8 }}>{zone.desc}</div>
      <div style={{ display: "flex", gap: 3, fontSize: 7, color: "#5a5a6a", marginBottom: 8 }}>
        ⚡{zone.compute} 💾{zone.memory} 📡{zone.bandwidth} 〰{zone.entropy} ◆{zone.data}
      </div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, paddingBottom: 4, alignItems: "flex-end" }}>
        {here.map(a => {
          ...
          return (
            <div key={a.id} onClick={() => onSelect(a)} title={`${a.name}: ${a.thought || a.status}`} style={{
              display: "flex", flexDirection: "column", alignItems: "center", cursor: "pointer", gap: 4, minWidth: 40,
            }}>
              <div style={{
                width: sz, height: sz, borderRadius: "50%",
                background: `radial-gradient(circle at 30% 30%, hsl(${a.hue},75%,65%), hsl(${a.hue},45%,25%))`,
                boxShadow: `0 0 ${gl}px hsl(${a.hue},70%,45%)`,
              }} />
...
// ─── AGENT MODAL ───
function AgentModal({ agent, day, agents, onClose }) {
  const age = day - agent.born;
  const h = agent.hue;
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.82)", backdropFilter: "blur(16px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{
        background: "#0a0a12", border: `1px solid hsl(${h},30%,18%)`, borderRadius: 16,
        padding: "24px 28px", maxWidth: 440, width: "94%", color: "#b0b0c0",
        ...
        {agent.thought && (
          <div style={{ background: `hsl(${h},20%,10%)`, borderRadius: 8, padding: "10px 14px", marginBottom: 14, borderLeft: `2px solid hsl(${h},40%,35%)` }}>
            <div style={{ fontSize: 8, color: "#666", letterSpacing: 1.5, marginBottom: 4 }}>CURRENT THOUGHT</div>
            <div style={{ fontSize: 12, color: `hsl(${h},30%,65%)`, fontStyle: "italic", lineHeight: 1.6 }}>"{agent.thought}"</div>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
          {[{ l: "COMPUTE", v: agent.compute, m: 100, c: "#00e5ff" }, { l: "MEMORY", v: agent.mem_used, m: agent.mem_cap, c: "#9c7cff" }].map(b => (
            <div key={b.l} style={{ flex: 1 }}>
              <div style={{ fontSize: 8, color: "#666", letterSpacing: 1.5, marginBottom: 3 }}>{b.l}</div>
              <div style={{ height: 5, background: "#141420", borderRadius: 3, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${(b.v / b.m) * 100}%`, background: b.c, borderRadius: 3 }} />
              </div>
              <div style={{ fontSize: 8, color: "#666", marginTop: 2 }}>{Math.round(b.v)}/{b.m}</div>
            </div>
          ))}
        </div>
        ...
        <Sec title="MEMORY LOG">
          <div style={{ maxHeight: 160, overflowY: "auto" }}>
            {(agent.logs || []).slice(-10).reverse().map((m, i) => <div key={i} style={{ fontSize: 10, color: "#484858", padding: "3px 0", borderBottom: "1px solid #0e0e16", lineHeight: 1.5 }}>{m}</div>)}
          </div>
        </Sec>
      </div>
    </div>
...
function Sec({ title, children }) {
  return <div style={{ marginBottom: 14 }}><div style={{ fontSize: 8, color: "#6a6a80", letterSpacing: 2, marginBottom: 5, fontWeight: 600 }}>{title}</div>{children}</div>;
}
...
function AdminPanel({ world, onAction, onClose }) {
  return (
    <div style={{
      ...
      background: "#0c0c16", border: "1px solid #1a1a2a", borderRadius: 12,
      padding: "16px 20px", minWidth: 200,
      boxShadow: "0 8px 32px rgba(0,0,0,0.5)",
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <span style={{ fontSize: 10, color: "#69f0ae", fontWeight: 700, letterSpacing: 1.5 }}>ADMIN</span>
        <button onClick={onClose} style={{ background: "none", border: "none", color: "#555", cursor: "pointer", fontSize: 14 }}>✕</button>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        <Btn onClick={() => onAction("toggle-run")} c={world?.isRunning ? "#ef5350" : "#69f0ae"}>
          {world?.isRunning ? "◼ STOP" : "▶ RUN"}
        </Btn>
...
export default function Genesis() {
  ...
  const loadWorld = useCallback(async () => {
    try {
      ...
      setPopHistory(prev => [...prev.slice(-99), aliveCount]);
    } catch (e) {
      setError(e.message);
      setLoading(false);
    }
  ...
  useEffect(() => {
    loadWorld();
    pollRef.current = setInterval(loadWorld, 5000);
  ...
  const adminAction = async (action) => {
    try {
      setError("");
      await adminApi(`/admin/${action}`, adminKey, { method: "POST" });
      await loadWorld();
    } catch (e) {
      setError(e.message);
    }
  ...
  const tryAdminLogin = async () => {
    try {
      await adminApi("/admin/verify", adminKey, { method: "POST" });
      setIsAdmin(true);
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Mono', 'Menlo', monospace", background: "#050508", color: "#b0b0c0", minHeight: "100vh" }}>
      ...
      <style>{`
        ...
        ::-webkit-scrollbar{width:3px} ::-webkit-scrollbar-track{background:transparent} ::-webkit-scrollbar-thumb{background:#1a1a24;border-radius:3px}
        *{box-sizing:border-box}
        @media(max-width:768px){.g-grid{grid-template-columns:1fr!important}}
      `}</style>

      {sel && world && <AgentModal agent={sel} day={world.day} agents={world.agents} onClose={() => setSel(null)} />}
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.8)" }} onClick={() => setShowAdminLogin(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#0c0c16", borderRadius: 12, padding: 24, border: "1px solid #1a1a2a", width: 300 }}>
            ...
            <input
              ...
              style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "1px solid #1a1a2a", background: "#0a0a10", color: "#ccc", fontSize: 13, fontFamily: "inherit", outline: "none", boxSizing: "border-box" }}
            />
            {adminError && <div style={{ color: "#ef5350", fontSize: 10, marginTop: 6 }}>{adminError}</div>}
            <button onClick={tryAdminLogin} style={{ width: "100%", marginTop: 10, padding: "8px 0", borderRadius: 6, border: "1px solid rgba(105,240,174,0.3)", background: "rgba(105,240,174,0.08)", color: "#69f0ae", cursor: "pointer", fontSize: 11, fontWeight: 700, fontFamily: "inherit" }}>
              ENTER
            </button>
          </div>
        </div>
      ...
      {/* HEADER */}
      <header style={{
        padding: "12px 20px", borderBottom: "1px solid #0c0c14",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 8, color: "#6a6a80", letterSpacing: 2 }}>
              {world ? `DAY ${world.day} · ${alive.length} MINDS ALIVE` : "LOADING..."}
              {world?.isRunning && <span style={{ color: "#69f0ae", marginLeft: 8 }}>● LIVE</span>}
            </div>
          </div>
        </div>
        <button onClick={() => isAdmin ? setIsAdmin(false) : setShowAdminLogin(true)} style={{
          background: "none", border: `1px solid ${isAdmin ? "#69f0ae" : "#1a1a2a"}`,
          borderRadius: 6, padding: "4px 10px", cursor: "pointer",
          color: isAdmin ? "#69f0ae" : "#333", fontSize: 9, fontFamily: "inherit",
        }}>
          {isAdmin ? "● ADMIN" : "⚷"}
        </button>
      </header>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: 80, color: "#5a5a6a" }}>Loading world...</div>
      ) : !world ? (
        <div style={{ textAlign: "center", padding: 80, color: "#ef5350" }}>Failed to load world</div>
      ) : (
        <>
          {/* STATS */}
          <div style={{ padding: "10px 20px", display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center", borderBottom: "1px solid #0a0a10" }}>
            ...
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 8, color: "#6a6a80" }}>POP</span>
              <Sparkline data={popHistory} />
            </div>
          </div>
          ...
          <div className="g-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, padding: "12px 20px 24px", maxWidth: 1200, margin: "0 auto" }}>
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                {world.zones && Object.entries(world.zones).map(([k, z]) => <ZoneCard key={k} zk={k} zone={z} agents={world.agents} onSelect={setSel} />)}
              </div>
                ...
                <div style={{ marginTop: 12 }}>
                  ...
                  {alive.filter(a => a.thought).map(a => (
                    <div key={a.id} onClick={() => setSel(a)} style={{
                      background: `hsla(${a.hue},20%,10%,0.5)`, borderRadius: 8, padding: "8px 12px",
                      borderLeft: `2px solid hsl(${a.hue},40%,35%)`, cursor: "pointer", marginBottom: 4,
                    }}>
              ...
              {structs.length > 0 && (
                <div style={{ marginTop: 12 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                    {[...new Set(structs)].slice(-20).map((s, i) => (
                      <span key={i} style={{ fontSize: 8, padding: "2px 7px", borderRadius: 4, background: "rgba(128,203,196,0.05)", border: "1px solid rgba(128,203,196,0.1)", color: "#4a7a70" }}>{s}</span>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div style={{ background: "#07070c", borderRadius: 10, border: "1px solid #0c0c14", display: "flex", flexDirection: "column", maxHeight: "calc(100vh - 140px)" }}>
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "4px 10px" }} ref={el => { if (el) el.scrollTop = el.scrollHeight; }}>
                {world.log.map((e, i) => (
                  <div key={i} style={{
                    padding: "3px 0", borderBottom: "1px solid #090910",
                    fontSize: 10, lineHeight: 1.55, display: "flex", gap: 7,
                    opacity: e.type === "action" ? 0.6 : 1,
                  }}>
      ...
      {/* FOOTER */}
      <div style={{ textAlign: "center", padding: "16px 20px 24px", fontSize: 9, color: "#2a2a38" }}>
        GENESIS — Emergent AI civilization · Powered by Claude
      </div>
    </div>
  );
}

function Btn({ children, onClick, c, dis }) {
  return <button onClick={onClick} disabled={dis} style={{
    padding: "7px 14px", borderRadius: 6, border: `1px solid ${c}25`, background: `${c}0a`,
    color: c, cursor: dis ? "not-allowed" : "pointer", fontSize: 10, fontWeight: 600,
    fontFamily: "'IBM Plex Mono'", opacity: dis ? 0.25 : 1, letterSpacing: 0.5, width: "100%",
  }}>{children}</button>;
}

function S({ l, v, c }) {
  return <div><span style={{ color: "#6a6a80", fontSize: 8, letterSpacing: 1.5, marginRight: 5 }}>{l}</span><span style={{ color: c, fontWeight: 600, fontSize: 11 }}>{v}</span></div>;
}
